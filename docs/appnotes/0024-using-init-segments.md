...
Some media formats, such as CMAF compatible MPEG-DASH and fMP4, split content into initialisation (init) segments and media segments.
...
One example being C2PA signing of live media.
...
2. Request storage for the init segment(s)
...
4. Request storage for the media segments
    * `content-type` MUST NOT be set
5. Upload the media segments
6. Register the media segments as Flow Segments
...
1. Get the Flow Segments listing for the Flow
